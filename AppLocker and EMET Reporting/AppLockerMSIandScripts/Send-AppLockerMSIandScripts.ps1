...
$error.clear()
$Events = Get-WinEvent -FilterHashtable @{LogName="Microsoft-Windows-AppLocker/MSI and Script";StartTime=$DateA;EndTime=$DateB} -ErrorAction Stop
 if ($error.Count -gt 0){
 exit
 }
 $y=1
...
$SqlCmd.CommandText = "sp_AddRecord_AppLocker_MSI_Part"
...
Else
{
$eventXML.save($MyDir + "\ErrEvents\" + $MachineName+"_"+ $B.Year+"_"+ $B.Month+"_"+ $B.day+"_"+ $B.hour+"_"+ $B.Minute +"_"+ $y+ "MSIerr_eventlog.xml")
}
